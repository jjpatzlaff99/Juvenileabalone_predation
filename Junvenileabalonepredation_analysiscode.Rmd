...
abpredation<-read.csv("Juvenile_abalone_predationms_SNISCI23_24_final.csv",header=T)
...
abpredationspacetot<-abpredation2 %>%  
  filter(!is.na(Predated.1)) %>% #filter out any rows where Predated = NA
...
abpredationspaceexclude<-abpredation2 %>%  
  filter(!is.na(Predated.1)) %>% #filter out any rows where Predated = NA
...
#exclude any irrelevant columns, filter out controls + nas
abpredation2 <- abpredation %>% 
  select(-c(8:9, 20, 21, 26:28, 32:34)) %>% 
  filter(!is.na(Predated.1)) %>% #filter out any rows where Predated = NA
  filter(Control=="Not control") 
view(abpredation2)
...
scale_fill_manual(values = c("San Clemente" = "gray50", "San Nicolas" = "white")) +
  scale_y_continuous(
    breaks = seq(0, 1, by = 0.20))+
  coord_cartesian(ylim = c(0, 1.10)) +
  labs(x = expression(paste("Crab density (# per ", m^2, ")")), y = "Proportion consumed") +
  theme_bw() +
  theme(
    axis.text = element_text(size = 10, color ="black", family = "Times New Roman"),
    axis.title = element_text(size = 18, color = "black", family = "Times New Roman"),
...
scale_fill_manual(values = c("San Clemente" = "gray50", "San Nicolas" = "white")) +
  scale_y_continuous(
    breaks = seq(0, 1, by = 0.20))+
  coord_cartesian(ylim = c(0, 1.10)) +
  labs(x = expression(paste("Slope (",degree,")")), y = "Proportion consumed") +
  theme_bw() +
  theme(
    axis.text = element_text(size = 10, color ="black", family = "Times New Roman"),
    axis.title = element_text(size = 18, color = "black", family = "Times New Roman"),
...
##Figure 3
```{r}
abshellpropzone$Shell_status<- factor(abshellpropzone$Shell_status, levels=c("Shell Intact", "Shell Missing", "Shell Damaged"), labels = c("Shell intact", "Shell removed", "Shell damaged"))
abshellpropzone
...
Figure_3a <- abshellpropzone %>%
 ggplot() +
  ...
  scale_y_continuous(
    breaks = seq(0, 1, by = 0.20))+
  coord_cartesian(ylim = c(0, 1.10)) +
  scale_fill_manual(values = c("Shell intact" = "white", "Shell removed" = "gray80", "Shell damaged"= "gray35")) +
  labs(
    x = "Zone",
    y = "Proportion",
    fill = "Tether remains"
...
abshellpropisland$Shell_status<- factor(abshellpropisland$Shell_status, levels=c("Shell Intact", "Shell Missing", "Shell Damaged"), labels = c("Shell intact", "Shell removed", "Shell damaged"))
